    ...
    webhook: async (req: Request, res: Response, next: Function) => {
        try {
            ...
            if (event.type === 'payment_intent.succeeded') {
                ...
                const existingTransaction = await TransactionRepo.query({ paymentId: intent.id, userId: userId as string })
                ...
                if (orderId) {
                    const order = await OrderRepo.Update({ orderId: orderId as string, paymentStatus: "paid", paymentMethod: "card" })
                    if (order) {
                        (async () => {
                            try {
                                if (order.items && order.items.length > 0) {
                                    ...
                                    const product = await ProductModel.findById(firstProductId).populate('createdBy');

                                    if (product && product.createdBy) {
                                        const business = product.createdBy as any;
                                        await sendEmail("business_notification", "New Order Received", {
                        ...
                        await sendEmail("transaction", "Payment Successful", {
                            userId: userId as string,
                            email: intent.receipt_email as string,
                            name: name as string
                        }, orderId as string).catch(e => console.error("Error sending user email:", e))
                    }
                } else if (intent.description === 'topup') {
                    await WalletRepo.topupUpdate(userId as string, intent.amount / 100)
                    await sendEmail("topup", "Payment Successful", {
                        userId: userId as string,
                        email: intent.receipt_email as string,
                        name: name as string
                    }, null).catch(e => console.error("Error sending topup email:", e))
                }
            }
    ...
    walletPayment: async (req: Request, res: Response, next: Function) => {
        try {
            ...
            if (walletBalance.balance.amount < order.totalAmount) {
                return res.status(400).json({ success: false, message: "Topup to update wallet or purchase product by card" })
            }

            await WalletRepo.decrement(userId, order.totalAmount)
            await OrderRepo.Update({ orderId: orderId, paymentStatus: "paid", paymentMethod: "wallet" })
